...
const Organization = () => {
  ...
  const subtitle = isLightMode ? 'subtitleL' : 'subtitle'
  const organizationP = isLightMode ? 'organizationPL' : 'organizationP';
  const howToUse = isLightMode ? 'howToUseL' : 'howToUse';
  const tips = isLightMode ? 'tipsL' : 'tips';


  return (
    <div>
      <HeaderOrganization />
      <div className="theme-toggle-containerOrg">
        <button className={`theme-toggle-buttonOrg ${isLightMode ? 'moon' : 'sun'}`} onClick={handleThemeToggle}>
          <img src={isLightMode ? moonIcon : sunIcon} alt="Alternar Tema" />
        </button>
      </div>
      <Principal />
      <h2 className={subtitle}>Bem-vindo ao nosso Planejador Diário!</h2>
      ...
      <h2 className={subtitle}>Dicas de organização:</h2>
      <ul className={tips}>
